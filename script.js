...
function isClaimToken(matchId) {
  return typeof matchId === "string" && matchId.startsWith("claim_");
}

function isOwnClaimToken(matchId) {
  if (!state.user?.uid || !isClaimToken(matchId)) return false;
  const parts = String(matchId).split("_");
  return parts[1] === state.user.uid;
}
...
function applyBattleOutcomeToCharacter({
  ...
  const finishedDateKey = getMoscowDateKey(finishedAt);
  let finalCreditGranted = Boolean(creditGranted);
  let finalCreditReason = creditReason || "";

  if (finalCreditGranted && next.training.progress.lastCreditedDateKey === finishedDateKey) {
    finalCreditGranted = false;
    finalCreditReason = finalCreditReason
      ? `${finalCreditReason} Уже есть зачёт за эти сутки.`
      : "Уже есть зачёт за эти сутки.";
  }
  ...
  next.training.progress.lastCreditedDateKey = finishedDateKey;
  next.training.progress.lastCreditedOpponentName = opponentName || "";
  next.training.progress.lastCreditedReason = finalCreditReason;
...
async function saveFinishedMatchIfNeeded(roomCode, room) {
  ...
  try {
    ...
    if (claimToken) {
      ...
      if (!claimed || !isOwnClaimToken(claimed)) return;
      claimToken = claimed;
    }
